...
- `hub_client` now honors the `hub` field in `~/.agent-mail/client.env`, so cockpit
  write operations can target a shared team hub while personal mode
  (`127.0.0.1:8765`) stays the default.
