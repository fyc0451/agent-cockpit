...
test('创建 Workspace happy path：严格 body + Idempotency-Key → 深链 workspace home', async ({
  ...
  const dialog = page.getByRole('dialog', { name: '创建工作空间' })
  await expect(dialog).toBeVisible()
  await dialog.getByLabel('工作空间名称').fill('E2E 工作区')
...
test('无合格 RepoLocation（p1）：按钮禁用 + reason 可见 + 点击零请求', async ({ page }) => {
  ...
  const btn = page.getByRole('button', { name: '创建工作空间' })
  await expect(btn).toHaveAttribute('aria-disabled', 'true')
  await expect(btn).toHaveAttribute('title', '这个项目还没有可用的代码目录。')
...
test('409 workspace_name_conflict：conflict 原地表达，不跳转', async ({ page }) => {
  ...
  await dialog.getByLabel('工作空间名称').fill('E2E 工作区')
  await dialog.getByRole('button', { name: '创建并打开' }).click()
  await expect(dialog.getByText('同名工作空间已存在').first()).toBeVisible()
  await expect(dialog.getByText('请换一个名称后再创建。')).toBeVisible()
  await expect(dialog).not.toContainText('Workspace')
  await expect(dialog).toBeVisible()
...
test('窄屏 390：向导可用且无水平溢出；取消零 POST', async ({ page }) => {
  ...
  const dialog = page.getByRole('dialog', { name: '创建工作空间' })
  await expect(dialog).toBeVisible()
  await dialog.getByLabel('工作空间名称').fill('半途')
  await expectNoHorizontalScroll(page)
...
test('wizard 在 workbench 之外零 legacy files 回退', async ({ page }) => {
  ...
  await expect(page.getByText('还没有工作空间')).toBeVisible()
  await page.getByRole('button', { name: '创建工作空间' }).click()
  await page.getByRole('dialog', { name: '创建工作空间' }).getByRole('button', { name: '取消' }).click()
  expect(apiCalls(g, '/api/files')).toEqual([])
