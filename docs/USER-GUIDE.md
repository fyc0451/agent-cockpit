...
开发页 →「＋ 添加 Agent」→ 选 session（可新建）/agent 类型/工作目录/任务（选填）/启动参数（选填）/布局 → 自动建 pane、启动、注册邮箱身份、通知协作者。工作区准备或启动失败会回滚；若仅身份注册/通知失败，Agent 会保留运行并在结果中显示警告，便于修好 Hub 后补注册。同类型可开多实例。会话整体管理在「工作台 → 会话」tab。
