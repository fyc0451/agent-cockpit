...
- `project_slug` is immutable after creation and exists for browser Hash routes
  and legacy API compatibility. New Project Registry APIs use only
  `project_id`.
- The server never guesses whether one path token is an ID or a slug. Legacy
  `/api/projects/{slug}` remains explicit; new routes use the existing `/api`
  prefix and live under `/api/project-registry/projects/{project_id}`. The
  distinct resource path prevents an ID/slug dispatch ambiguity without
  creating a parallel versioned API.
