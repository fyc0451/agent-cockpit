...
def test_legacy_v2_inventory_accepts_exact_sixteen_entry_catalog(tmp_path):
    ...
    work = next(
        row for row in evidence["stores"] if row["name"] == "workspace_work"
    )
...
def test_legacy_v3_inventory_accepts_exact_twenty_one_entry_catalog(tmp_path):
    snapshot = _make_snapshot(tmp_path)
    inventory = _inventory(schema_version=3)
    ...
    work = next(
        item for item in evidence["stores"] if item["name"] == "workspace_work"
    )
    assert work["state"] == "absent"
    assert work["reason"] == store_schema.REASON_MISSING_CREATABLE
