...
def test_project_registry_is_a_closed_inventory_sqlite_store() -> None:
    ...
    assert upgrade_snapshot.INVENTORY_SCHEMA_VERSION == 5
    assert len(upgrade_snapshot.SQLITE_STORE_NAMES) == 13
    assert len(upgrade_snapshot.SNAPSHOT_STORE_NAMES) == 20
    ...
    assert upgrade_snapshot._EXPECTED_STORE_LAYOUT["workspace_execution"] == (
        "data", "workspace-execution.sqlite3", "file",
    )
    assert "workspace_execution" in upgrade_snapshot.SQLITE_STORE_NAMES
    assert "workspace_execution" in upgrade_snapshot.SNAPSHOT_STORE_NAMES
    expected = {
    ...
    assert "workspace_execution" not in expected
...
def test_empty_sources_seal_exact_canonical_closed_inventory(
    ...
    assert inventory["entry_count"] == 23
